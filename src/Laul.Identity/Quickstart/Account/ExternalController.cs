        ...
        private async Task<IdentityUser> AutoProvisionUser(string provider, string providerUserId, IEnumerable<Claim> claims)
        {
            ...
            if (user == null)
            {
                ...
                foreach (var claim in claims)
                {
                    ...
                    {
                        user.Email = claim.Value; // Встановити Email на основі Email з Claims
                    }
                    // Додайте інші перевірки та заповнення властивостей за потребою
                }

                // Збережіть користувача у базі даних
                ...
                result = await _signInManager.UserManager.AddLoginAsync(user, userLoginInfo);

                if (!result.Succeeded)
                {
                    // Обробити помилку додавання зв'язку за потреби
